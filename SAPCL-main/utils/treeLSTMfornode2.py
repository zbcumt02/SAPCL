"""
在treeLSTMfornode基础上针对偏标记数据的存在进行消融实验
"""
import json
import torch
import numpy as np
...
class phemes_Augmentention(Dataset):
...
class phemes_test_Augmentention(Dataset):
...
def label_onehot(label_set, partial_rate=0.1):
    train_label_set_onehot = []
    for label_truth_index in range(label_set.shape[0]):
        label_truth = label_set[label_truth_index]
        label_one_hot = torch.zeros([1, 5])
        set_theta = 1 - partial_rate
        label_one_hot[0, label_truth] = 1
        # random_number1 = random.random()
        # if label_one_hot[0, 2] == 1 and random_number1 > set_theta:
        #     label_one_hot[0, 3] = 1
        # random_number2 = random.random()
        # if label_one_hot[0, 3] == 1 and random_number2 > set_theta:
        #     label_one_hot[0, 2] = 1
        train_label_set_onehot.append(label_one_hot)
    train_label_set_onehot = torch.cat(train_label_set_onehot, dim=0)
    return train_label_set_onehot


def load_phemes(partial_rate, batch_size):
    test_ratio = 0.2
    ...
    train_feature = np.concatenate((np.load('D:/PycharmProjects/25-3-zb1/SAPCL-main/data/phemes/' + five_events[0] + "deberta-v3.npy"),
    ...
    dataset = phemesDataset(data, label_arr)
    # 计算训练集和测试集的划分大小
    ...
    partialY = label_onehot(train_labels, partial_rate)

    partial_matrix_dataset = phemes_Augmentention(train_data, partialY.float(), train_labels.float())
    test_matrix_dataset = phemes_test_Augmentention(test_data, test_labels.float())
...
#     load_phemes(partial_rate=0.05, batch_size=64)
