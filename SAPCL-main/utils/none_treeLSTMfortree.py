...
class TreeLSTMCell(nn.Module):
    def __init__(self, input_size=768, hidden_size=64, class_num=5):
    ...
    def node_forward(self, inputs, children_states):
        ...
        h_reshaped = h.view(batch_size, 8, 8)
        ...
        # 存储输出状态
        self.hidden_buffer.append(output)  # 将输出添加到缓存中
        return (h, c)

    def get_hidden_buffer(self, inputs, tree, current_child_id):
        self.hidden_buffer = []
        self.forward(inputs, tree, current_child_id)
        output = torch.stack(self.hidden_buffer).mean(dim=0, keepdim=True)
        return output
...
class tweets_Augmentention(Dataset):
    ...
    def __getitem__(self, index):
        each_image_w = self.dataset[index]
        each_image_s = self.dataset[index]
...
def load_tweets(partial_rate, batch_size):
    ...
    output_combined = torch.zeros(0, 8, 8)
    ...
    data = torch.cat((data, output_combined), dim=0)
    print(output_combined.shape)
    print(output_combined[0])
    print(data.shape)


    # return train_loader, train_label_set_onehot, train_sampler, test_loader


load_tweets(partial_rate=0.05, batch_size=1)