...
class pheme_test_combine(Dataset):
...
class pheme_Augmentention(Dataset):
...
def load_pheme(partial_rate, batch_size):
    test_ratio = 0.2
    ...
    for index in train_index_set:
        output = get_tree(train_feature[index], whole_tree[index]['0'])  #shape(n, 768)
        output_combined = torch.cat((output_combined, output), dim=0)

    data = output_combined  # 4612*768
    dataset = phemeDataset(data, label_arr)
    # 计算训练集和测试集的划分大小
    ...
    # partialY = torch.zeros([train_labels.size(0), 5])
    ...
    # test_labels_onehot = one_hot(test_labels, 5)
    ...
    #
    # # 保存为 .mat 文件
    # scipy.io.savemat('pheme_dataset.mat', {
    ...
    partial_matrix_dataset = pheme_Augmentention(train_data, partialY.float(), train_labels.float())
    test_matrix_dataset = pheme_test_combine(test_data, test_labels.float())
...
#     load_pheme(partial_rate=0.05, batch_size=64)
