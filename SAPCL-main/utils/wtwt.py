...
class wtwts_test_combine(Dataset):
...
class wtwts_Augmentention(Dataset):
...
def load_wtwt(partial_rate, batch_size):
    test_ratio = 0.2

    label_set = []
    label_arr = []

    train_label_in = open('data/wtwt/stance_labels.txt', 'r')
    for i, line in enumerate(train_label_in):
        # if i < 33000:
        #     continue
        # if i >= 2500:
        #     break
    ...
    train_feature = np.load('data/wtwt/text_vectors.npy')
    # train_feature = train_feature[:2500]  # 33000
    # shape(51284, 768)
    print(len(label_set), len(train_feature))
    ...
    dataset = wtwtsDataset(data, label_arr)
    # 计算训练集和测试集的划分大小
    ...
    # partialY = torch.zeros([train_labels.size(0), 4])
    ...
    # test_labels_onehot = one_hot(test_labels, 4)
    ...
    #
    # # 保存为 .mat 文件
    # scipy.io.savemat('wtwt_dataset.mat', {
    ...
    partial_matrix_dataset = wtwts_Augmentention(train_data, partialY.float(), train_labels.float())
    test_matrix_dataset = wtwts_test_combine(test_data, test_labels.float())
...
#     load_wtwt(partial_rate=0.05, batch_size=64)
